...
@given(data())
def t_core_svc_requester(fi, cgs, maxSvcReqs, dump, data):
    """
    ...
    print("\n\n@@@@@@@@ Start test :", cgs.testCount, ", nServers:", nServers,
    ...
    print("\n\n***** Start Simulation (%s, %s, %s)-%s *****" % (
        nServers, nSvcRqrs, dump, cgs.testCount), file=fi)
    ...
    print("@@@@@@@@ End test: " + str(cgs.testCount) + " ended: " + str(env.now),
          file=fi)
...
def test_core_svc_requester1(fi, maxServers, maxSvcRqrs, maxSvcReqs, dump):
    ...
    t_core_svc_requester(fi, cgs, maxSvcReqs, dump)
